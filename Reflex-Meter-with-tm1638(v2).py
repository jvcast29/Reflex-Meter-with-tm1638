...
while True:
    ...
    if keys_crudas != 0 and tecla_anterior == 0:
        if time.ticks_diff(tiempo_actual, tiempo_ultimo_rebote) > 150:
            ...
            print (f"Pulsación válida: 0x{evento_tecla:X}")
    # ---------------------------------------------------------
    # MÁQUINA DE ESTADOS FINITOS (FSM)
    # ---------------------------------------------------------           
    tecla_anterior = keys_crudas
    if estado_juego == "BIENVENIDA":
        # Pausa inicial mostrando el saludito
        if time.ticks_diff(tiempo_actual, inicio) > 10000:
            estado_juego = "INICIO"
    ...
    time.sleep_ms(5) # Watchdog y estabilización del bus SPI
